...
def analizza_governance(risposte):
    ...
    # GDPR — legal basis
    if risposte.get("base_giuridica") == "unknown":
        problemi.append({t("col_problem"): t("gov_legal_basis_unknown"), t("col_detail"): t("gov_legal_basis_unknown_detail"), t("col_severity"): t_level(LEVEL_HIGH)})

    # GDPR — special categories / explicit consent
    if risposte.get("categorie_speciali") == "yes":
        if risposte.get("consenso_esplicito") == "no":
            problemi.append({t("col_problem"): t("gov_explicit_consent_missing"), t("col_detail"): t("gov_explicit_consent_missing_detail"), t("col_severity"): t_level(LEVEL_HIGH)})
        elif risposte.get("consenso_esplicito") == "unknown":
            problemi.append({t("col_problem"): t("gov_explicit_consent_unknown"), t("col_detail"): t("gov_explicit_consent_unknown_detail"), t("col_severity"): t_level(LEVEL_MEDIUM)})

    # GDPR — AI usage notice
    if risposte.get("informativa_ai") == "no":
        problemi.append({t("col_problem"): t("gov_ai_notice_missing"), t("col_detail"): t("gov_ai_notice_missing_detail"), t("col_severity"): t_level(LEVEL_HIGH)})
    elif risposte.get("informativa_ai") == "unknown":
        problemi.append({t("col_problem"): t("gov_ai_notice_unknown"), t("col_detail"): t("gov_ai_notice_unknown_detail"), t("col_severity"): t_level(LEVEL_MEDIUM)})
